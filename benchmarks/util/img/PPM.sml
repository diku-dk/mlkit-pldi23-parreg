...
sig
  ...
  type image = {height: int, width: int, data: pixel Array.array}
...
struct
  ...
  fun subimage {topleft=(i1,j1), botright=(i2,j2)} image =
    ...
    in
      { width = w
      , height = h
      , data = Array.tabulate (w * h, newElem)
      }
    end
  ...
  fun replace {topleft=(i1,j1), botright=(i2,j2)} image subimage =
    ...
    in
      { width = #width image
      , height = #height image
      , data = Array.tabulate (#width image * #height image, newElem)
      }
    end
